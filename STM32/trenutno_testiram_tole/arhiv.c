 ...
 *
 *  A1 = PF8  → IN1: TIM13_CH1 (AF9)  – H-bridge input 1
 *  A2 = PA0  → IN2: TIM2_CH1  (AF1)  – H-bridge input 2
 *       Forward  : IN1 = PWM,  IN2 = 0
 *       Reverse  : IN1 = 0,    IN2 = PWM
 *
 *  A3 = PA1  → direction-change button, EXTI1, active LOW + pull-up
 *
 *  Note: PA0_C is the analog-only companion pad; use PA0 (digital) for PWM.
 */
#define DC_IN1_PORT         GPIOF
#define DC_IN1_PIN          GPIO_PIN_8          /* A1 – TIM13_CH1 AF9 */
#define DC_IN2_PORT         GPIOA
#define DC_IN2_PIN          GPIO_PIN_0          /* A2 – TIM2_CH1  AF1 */
...
static TIM_HandleTypeDef       htim2;   /* TIM2_CH1  → IN2 on PA0 (A2) */
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin)
{
    ...
    if((current_time - last_interrupt_time > 50)) {
        ...
        switch(GPIO_Pin) {
            ...
            case GPIO_PIN_1:
                if (htim13.Instance != NULL && htim2.Instance != NULL) {
...
void DC_Motor_Update(uint16_t distance_mm)
{
    ...
    if (distance_mm < DC_DIST_MIN_MM) {
        ...
        /* in range → linear 0–100 % based on distance */
    ...
    /* X-NUCLEO-IHM04A1: IN1 and IN2 are complementary PWM inputs.
     * Forward : IN1 = PWM, IN2 = 0
     * Reverse : IN1 = 0,   IN2 = PWM
...
void DC_Motor_Init(void)
{
   ...
   // HAL_Delay(10);
    ...
    /* ---------------------------------------------------------------
     * TIM2 – IN2 PWM (PA0, A2)
     * TIM2 is a 32-bit timer on APB1, same clock domain
     * --------------------------------------------------------------- */
